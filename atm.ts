...
const predefinedUserID = "Sahir Ahmed Sheikh"; // Define the specific User ID
const predefinedPin = 4321; // Define the specific PIN
...
async function promptForTransaction() {
    ...
    if (userInput.transactionType === "Balance Inquiry") {
        ...
        if (accountBalance >= enteredAmount) {
            ...
            console.log(`Your account has been debited with Rs ${enteredAmount} and your remaining balance is Rs ${accountBalance}`);
        } else {
    ...
    const { anotherTransaction }: { anotherTransaction: string } = await inquirer.prompt([
        {
            type: "confirm",
